...
class Feeds extends React.Component {
  ...
  async getWedeyyFeeds() {
    const api = new API();
    let feeds = await api.getWedeyyFeeds(this.state.peopleRSN);
    return feeds;
  }

  async componentDidMount() {
    let feeds = await this.getWedeyyFeeds();
    let sfb = randomNumber(1, feeds.length);
    this.setState({
      feeds: feeds,
      feedsLoaded: true,
      suggestFollowBegin: sfb
    });
  }
              ...
              return (
                <div key={feed.ID + index + "div"}>
                  <Feed
                    ...
                    contentType="image"
                    likeCount={abbreviateNumber(feed.LIKES)}
